...
$query = "SELECT p.panne_num, p.panne_name, p.date_signalement, 
          COALESCE(e.etablissement_name, 'UNITE-POSTAL-WILAYA-DE-BOUMERDES') AS etablissement_name, 
          t.type_name, p.panne_etat, r.rap_num, r.rap_name 
          FROM Panne p 
...
if ($_SESSION['user_role'] == 'Receveur') {
    $query = "SELECT p.panne_num, p.panne_name, p.date_signalement, e.etablissement_name, t.type_name, p.panne_etat, r.rap_num, r.rap_name 
              FROM Panne p 
              INNER JOIN Type_panne t ON p.type_id = t.type_id 
              INNER JOIN Users u ON p.receveur_id = u.user_id 
